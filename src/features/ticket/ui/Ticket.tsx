...
export const Ticket = ({ ticket, currency, exchangeRates }: TicketProps) => {
  ...
  const visiblePrice =
    rate !== undefined
      ? Math.round(ticket.price * rate).toLocaleString()
      : 'Loading...';
    ...
    <div className="flex max-w-lg min-w-72 max-h-[146px] md:max-h-[120px] flex-col-reverse md:flex-row items-center border rounded-lg shadow-md p-1 bg-white mb-4">
      <div className="md:w-[30%] w-[80%] md:h-auto flex md:flex-col flex-row justify-between items-center md:border-r md:border-t-0 border-t p-1">
        <img
          ...
          alt={ticket.carrier}
          className="w-[40%] md:min-w-[90%] md:h-[50%]"
        />
        <button className="md:min-w-[90%] w-[40%] text-[9px] md:text-sm bg-orange text-white font-semibold py-2 px-2 mt-1 md:mt-2 rounded-lg hover:bg-orange-dark transition">
          Купить
          <br />
          за {visiblePrice}
      ...
      <div className="flex md:w-[70%] md:h-auto w-[90%] flex-row justify-between text-lg font-semibold p-2">
        <div className="flex flex-col">
          <span className="md:text-3xl text-xl text-gray-600">
            {' '}
            {ticket.departure_time}
          </span>
          <span className="text-[11px] whitespace-nowrap text-gray-600 md:mt-2 mt-1">
            {ticket.origin}, {ticket.origin_name}
          </span>
          <span className="text-[11px] whitespace-nowrap text-gray-400 md:mt-2 mt-1">
            {formatDate(ticket.departure_date)}
          </span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-[7px] whitespace-nowrap mb-1">
            {ticket.stops === 0 ? 'Прямой' : `${ticket.stops} пересадка`}
          </span>
          <LineWithArrow />
        </div>
